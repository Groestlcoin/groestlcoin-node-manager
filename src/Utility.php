...
function getCleanClient($client){
	...
	if(preg_match("/^Groestlcoin:([0]\.[0-9]{1,2}\.[0-9]{1,2})/",$client, $matches)) {
		$client = "Core ".$matches[1];
	}elseif(preg_match("/MultiGroestlHD:([0-9]\.[0-9]{1,2}\.[0-9]{1,2})/i",$client, $matches)){
		$client = "MultiGroestl HD ".$matches[1];
	}elseif(preg_match("/Multigroestl:([0-9]\.[0-9]{1,2}\.[0-9]{1,2})/i",$client, $matches)){
		$client = "MultiGroestl ".$matches[1];
	}elseif(preg_match("/Groestlcoin Wallet:([0-9]\.[0-9]{1,2})/i",$client, $matches)){
		$client = "Groestlcoin Wallet ".$matches[1];
	}else{
...
function checkSPV($client){
	if (preg_match('/MultiGroestl|groestlcoinj|bread/i',$client)){
		return true;
	}else{
		return false;
	}
    ...
    // Checks if we know about the oldest peer, if not we assume that we don't known any peer
		foreach($arrayPeers as $key => $peer){
			if($oldestPeerIp == $peer[0]){
				$delete = true;
				// Either groestlcoind was restarted or peer reconnected. Since peer is the oldest, all other peers we known disconnected
				if($oldestPeerId != $key){
					$delete = false;
				}
				break;
